...
export async function POST(request: NextRequest, { params }: RouteContext) {
  const context = createRequestContext(request, "like-post");

  if (!hasSupabaseServerConfig) {
    return jsonWithRequestContext({ error: "Likes require Supabase configuration." }, context, { status: 503 });
  }
  ...
  if (!viewer) {
    return jsonWithRequestContext({ error: "Sign in before reacting." }, context, { status: 401 });
  }
  ...
  if (error && !error.message.toLowerCase().includes("duplicate")) {
    logServerEvent("warn", context, "like failed", { postId: id, error: error.message });
...
export async function DELETE(request: NextRequest, { params }: RouteContext) {
  const context = createRequestContext(request, "unlike-post");
  ...
  const { id } = await params;
  const supabase = await createClient();
  const { error } = await supabase.from("reactions").delete().eq("post_id", id).eq("profile_id", viewer.profileId).eq("reaction_type", "like");
  if (error) {
    logServerEvent("warn", context, "unlike failed", { postId: id, error: error.message });
    return jsonWithRequestContext({ error: error.message }, context, { status: 400 });
  }
